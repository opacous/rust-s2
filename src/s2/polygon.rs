...
impl Polygon {
    ...
    fn PolygonFromLoops(loops: [] * Loop) -> * Polygon {
        p: = &Polygon {}
    ...
    // inverted.)
    fn PolygonFromOrientedLoops(loops: [] * Loop) -> * Polygon {
    ...
    * p = Polygon{}
    ...
    // order of loop vertices. This fntion is used to choose which loop to
    // invert in the case where several loops have exactly the same area.
    fn compareLoops(a, b *Loop)i64 {
    ...
    fn PolygonFromCell(cell Cell) *Polygon {
    ...
    // a different level.
    fn (lm loopMap) insertLoop(newLoop, parent *Loop) {
    var children [] * Loop
    for done: = false; ! done; {
    children = lm[parent]
    done = true
    ...
    fn (s *loopStack) push(v *Loop) {
    * s = append( * s, v)
    }
    fn (s *loopStack) pop() * Loop {
    l : = len( * s)
    r : = ( * s)[l-1]
    * s = ( * s)[: l-1]
    return r
    ...
    fn  findLoopNestingError( & mut self ) -> error {
    ...
    fn  NumLoops( & mut self ) -> i64 {
    return len(p.loops)
    }
    
    // Loops returns the loops in this polygon.
    fn  Loops( & mut self ) -> [] * Loop {
    return p.loops
    }
    ...
    // to Loop(k).depth+1.
    fn  LastDescendant( & mut self , k: i64) -> i64 {
    if k < 0 {
    ...
    fn  CapBound( & mut self ) -> Cap { return p.bound.CapBound() }
    
    // RectBound returns a bounding latitude-longitude rectangle.
    fn  RectBound( & mut self ) -> Rect { return p.bound }
    
    // ContainsPoint reports whether the polygon contains the point.
    fn  ContainsPoint( & mut self , point Point) -> bool {
    // NOTE: A bounds check slows down this fntion by about 50%. It is
    ...
    fn  ContainsCell( & mut self , cell Cell) -> bool {
    it : = p.index.Iterator()
    ...
    fn  IntersectsCell( & mut self , cell Cell) -> bool {
    it : = p.index.Iterator()
    relation : = it.LocateCellID(cell.ID())
    ...
    fn  boundaryApproxIntersects( & mut self , it *ShapeIndexIterator, cell Cell) -> bool {
    ...
    fn  iteratorContainsPoint( & mut self , it *ShapeIndexIterator, point Point) -> bool {
    ...
    fn  NumEdges( & mut self ) -> i64 {
    return p.numEdges
    }
    
    // Edge returns endpoints for the given edge index.
    fn  Edge( & mut self , e i64) -> Edge {
    var i i64
    ...
    fn  ChainEdge( & mut self , i, j i64) -> Edge {
    ...
    fn  typeTag( & mut self ) -> typeTag { return typeTagPolygon }
    ...
    fn  Contains( & mut self , o *Polygon) -> bool {
    ...
    fn  Intersects( & mut self , o *Polygon) -> bool {
    // If both polygons have one loop, use the more efficient Loop method.
    ...
    fn  containsBoundary( & mut self , o *Polygon) -> bool {
    for _, l: = range o.loops {
    ...
    fn  excludesBoundary( & mut self , o *Polygon) -> bool {
    for _, l: = range o.loops {
    if p.compareBoundary(l) > = 0 {
    return false
    }
    ...
    fn  containsNonCrossingBoundary( & mut self , o *Loop, reverse bool) -> bool {
    var inside bool
    for _, l : = range p.loops {
    ...
    fn  excludesNonCrossingShells( & mut self , o *Polygon) -> bool {
    for _, l: = range o.loops {
    if l.IsHole() {
    continue
    }
    ...
    fn  excludesNonCrossingComplementShells( & mut self , o *Polygon) -> bool {
    ...
    fn  anyLoopContains( & mut self , o *Loop) -> bool {
    for _, l: = range p.loops {
    if l.Contains(o) {
    return true
    }
    ...
    fn  anyLoopIntersects( & mut self , o *Loop) -> bool {
    for _, l: = range p.loops {
    if l.Intersects(o) {
    return true
    }
...
// const pointSize = 3 * 8                    // s2.Point is an r3.Vector, which is 3  f64s. That's 3*8 = 24 bytes.
...
// }
// 
// // encodeLossless encodes the polygon's Points as  f64s.
// fn  encodeLossless(e *encoder) {
...
// }
// 
// fn  encodeCompressed(e *encoder, snapLevel int, vertices []xyzFaceSiTi) {
